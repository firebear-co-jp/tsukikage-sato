// 予約システム用のデバッグ版Google Apps Scriptコード
...
function handleReserve(callback, data) {
  try {
    ...
    // 予約をスプレッドシートに追加
    const reservationId = addReservation(data, selectedRoom);
...
// 利用可能な部屋を計算
function calculateAvailableRooms(conflictingReservations, guests) {
...
// 予約をスプレッドシートに追加
function addReservation(data, selectedRoom) {
  try {
    const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = spreadsheet.getSheetByName('予約');
...
function sendReservationConfirmationEmail(data, selectedRoom, reservationId) {
  try {
    ...
    const emailSubject = `[月影の郷] ご予約確認: ${reservationId}`;
    const emailBody = `
${data.name} 様
...
function sendAdminNotificationEmail(data, selectedRoom, reservationId) {
  try {
    ...
    const emailSubject = `[月影の郷] 新しい予約: ${reservationId}`;
    const emailBody = `
新しい予約が入りました。
